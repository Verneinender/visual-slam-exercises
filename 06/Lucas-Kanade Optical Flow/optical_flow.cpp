...
void OpticalFlowSingleLevel(
    ...
    for (size_t i = 0; i < kp1.size(); i++) {
        auto kp = kp1[i];// check the same keypoint
...
void OpticalFlowMultiLevel(
    ...
    for (int i = 0; i < pyramids; i++) {
        ...
        resize(img1, img1_pyr, Size(img1.size().width*scales[i], img1.size().height*scales[i]));
        resize(img2, img2_pyr, Size(img2.size().width*scales[i], img2.size().height*scales[i]));
