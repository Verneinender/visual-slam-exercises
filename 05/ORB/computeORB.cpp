...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
        ...
        int y = kp.pt.y;
        double m01=0, m10=0;
        if(x >=8 && y >= 8 && x + 7 <= image.cols && y + 7 <= image.rows ) {
            for(int u = -8; u <= 7; ++u) {
                for(int v = -8; v <= 7; ++v) {
                    m01 += v*image.at<uchar >(y+v,x+u); //注意这里的坐标，（y，x）
                    m10 += u*image.at<uchar >(y+v,x+u); //图像矩中的坐标为图像块的像素坐标
                }

            }
            kp.angle = std::atan2(m01, m10) * (180/pi); // compute kp.angle 
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints) {
        ...
        for (int i = 0; i < 256; i++) {
            // START YOUR CODE HERE (~7 lines)
            //int p_relative [] = {ORB_pattern[4*i], ORB_pattern[4*i+1]};
            //int q_relative [] = {ORB_pattern[4*i+2], ORB_pattern[4*i+3]};
            auto cos_ = float(cos(kp.angle*pi/180)); //将角度转换成弧度再进行cos、sin的计算
            auto sin_ = float(sin(kp.angle*pi/180));
            cv::Point2f p_relative = {cos_*ORB_pattern[4*i] - sin_*ORB_pattern[4*i+1],
                                      sin_*ORB_pattern[4*i] + cos_*ORB_pattern[4*i+1]};
            cv::Point2f q_relative = {cos_*ORB_pattern[4*i+2] - sin_*ORB_pattern[4*i+3],
                                      sin_*ORB_pattern[4*i+2] + cos_*ORB_pattern[4*i+3]};

            cv::Point2f p = {kp.pt + p_relative};
            cv::Point2f q = {kp.pt + q_relative};


            if(p.x >= 0 && p.x <= image.cols && p.y >= 0 && p.y <= image.rows 
            && q.x >= 0 && q.x <= image.cols && q.y >= 0 && q.y <= image.rows ) {
