...
double RMSE(const vector<Sophus::SE3, Eigen::aligned_allocator<Sophus::SE3>> v1, const vector<Sophus::SE3, Eigen::aligned_allocator<Sophus::SE3>> v2);
...
int main(int argc, char **argv) {
    ...
    */
    ...
    double root_mean_square_error = RMSE(poses_groundtruth, poses_estimated);
    cout << root_mean_square_error << endl;
...
void read_estimated(vector<Sophus::SE3, Eigen::aligned_allocator<Sophus::SE3>> & vv) {
    ifstream fin_estimated;
    fin_estimated.open(estimated_file);
        if(!fin_estimated) {
        cout<<"file not found!"<<endl;
    }
...
void read_groundtruth(vector<Sophus::SE3, Eigen::aligned_allocator<Sophus::SE3>> & vv) {
    ifstream fin_groundtruth;
    fin_groundtruth.open(groundtruth_file);
        if(!fin_groundtruth) {
        cout<<"file not found!"<<endl;
    }
...
double RMSE(const vector<Sophus::SE3, Eigen::aligned_allocator<Sophus::SE3>> v1, const vector<Sophus::SE3, Eigen::aligned_allocator<Sophus::SE3>> v2) {
    ...
    //Vector6d se3_groundtruth = v1.log();
    //Vector6d se3_estimated = v2.log();
    auto ig = v1.begin();
    auto ie = v2.begin();
    Vector6d e_diff;
    double ei_squared, sum, RMSE;
    while(ig != v1.end() && ie != v2.end()) {
        e_diff = ((*ig).inverse() * (*ie)).log();
        ei_squared = e_diff.squaredNorm();
        sum = sum + ei_squared;
        ++ig;
        ++ie;
    }
    RMSE = sqrt(sum/double(v1.size()));
    return RMSE;
